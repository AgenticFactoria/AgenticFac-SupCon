...
class ProductTracking:
    ...
    production_start_time: Optional[float] = (
        None  # When it actually enters production (StationA)
    )
...
class KPICalculator:
    ...
    def __init__(
        ...
        self.efficiency_weights = kpi_weights.get(
        ...
        self.quality_cost_weights = kpi_weights.get(
            "quality_cost_components",
            {"first_pass_rate": 0.40, "cost_efficiency": 0.60},
        )

        self.agv_weights = kpi_weights.get(
            "agv_components",
            {"charge_strategy": 0.30, "energy_efficiency": 0.40, "utilization": 0.30},
        )

        # Load cost parameters from config
        kpi_costs = config.get("kpi_costs", {})
        self.cost_parameters = {
            "material_cost_per_product": kpi_costs.get(
                "material_cost_per_product", {"P1": 10.0, "P2": 15.0, "P3": 20.0}
            ),
            "energy_cost_per_second": kpi_costs.get("energy_cost_per_second", 0.1),
            "energy_cost_multiplier_peak": kpi_costs.get(
        ...
        self.theoretical_production_times = order_gen_config.get(
            "theoretical_production_times", {"P1": 160.0, "P2": 200.0, "P3": 250.0}
        )
        ...
        for item in order.items:
            ...
            for i in range(item.quantity):
                ...
                product_tracking = ProductTracking(
                    ...
                    theoretical_time=self.theoretical_production_times[
                        item.product_type
                    ],
                )
    ...
    def complete_order_item(
        ...
        if product_type:
            for product in order.products:
                if product.product_type == product_type and product.end_time is None:
                    ...
                    actual_start = (
                        product.production_start_time
                        if product.production_start_time is not None
                        else product.start_time
                    )
                    ...
                    if theoretical_time > 0 and passed_quality:
                        self.stats.weighted_production_cycle_sum += (
        ...
        if passed_quality:
            ...
            if product_type:
                scrap_cost = (
                    self.cost_parameters["material_cost_per_product"][product_type]
                    * self.cost_parameters["scrap_cost_multiplier"]
                )
    ...
    def add_energy_cost(
        ...
        try:
            if not isinstance(duration, (int, float)):
                raise TypeError(
                    f"Duration must be a number, got {type(duration).__name__}: {duration}"
                )
        except Exception:
            print(
                f"[DEBUG] add_energy_cost called with: device_id={device_id}, line_id={line_id}, duration={duration}, is_peak_hour={is_peak_hour}"
            )
            raise
    ...
    def add_maintenance_cost(
        self, _device_id: str, _maintenance_type: str, was_correct_diagnosis: bool
    ):
    ...
    def mark_production_start(self, product: "Product"):
        ...
        for tracking_id, tracking in self.active_products.items():
            if (
                ...
                and tracking.product_type == product_type
                and tracking.production_start_time is None
            ):
                tracking.production_start_time = self.env.now
                print(
                    f"[KPI] Product {product.id} (tracking: {tracking_id}) started production at {self.env.now:.2f}"
                )
                break

    def register_agv_charge(
        self,
        agv_id: str,
        line_id: Optional[str],
        is_active: bool,
    ...
    def update_agv_transport_time(
        self, agv_id: str, line_id: Optional[str], transport_time: float
    ):
    ...
    def update_agv_fault_time(
        self, agv_id: str, line_id: Optional[str], fault_time: float
    ):
    ...
    def update_device_utilization(
        self, device_id: str, line_id: Optional[str], total_time: float
    ):
    ...
    def track_device_working_time(
        self, device_id: str, line_id: Optional[str], duration: float
    ):
    ...
    def calculate_current_kpis(self) -> KPIUpdate:
        ...
        base_cycle_ratio = (
            (
                self.stats.weighted_production_cycle_sum
                / self.stats.quality_passed_products
            )
        ...
        total_production_cost = (
            self.stats.material_costs
            + self.stats.energy_costs
            + self.stats.maintenance_costs
            + self.stats.scrap_costs
        )
        ...
        charge_strategy_efficiency = (
            ...
            if total_charges > 0
            else 0.0  # No charging activity = 0 efficiency
        )
        ...
        agv_energy_efficiency = (
            (
                self.stats.agv_completed_tasks / self.stats.agv_total_charge_time
            )  # tasks per second of charging
            if self.stats.agv_total_charge_time > 0
            else 0.0
        )
    ...
    def _publish_kpi_update(self, kpi_update: KPIUpdate):
        ...
        try:
            if self.mqtt_client:
                ...
                self.mqtt_client.publish(topic, kpi_update.json())
    ...
    def get_final_score(self) -> Dict[str, Any]:
        ...
        if self.stats.quality_passed_products > 0 and kpis.average_production_cycle > 0:
            # 比率越接近1越好（实际时间接近理论时间）
            production_cycle_score = min(
                100, 100 / max(1, kpis.average_production_cycle)
            )
        else:
            # 没有生产任何产品，周期效率为0
        ...
        # 子权重分配
        efficiency_score = (
            efficiency_components["order_completion"]
            * self.efficiency_weights.get("order_completion", 0.4)
            + efficiency_components["production_cycle"]
            * self.efficiency_weights.get("production_cycle", 0.4)
            + efficiency_components["device_utilization"]
            * self.efficiency_weights.get("device_utilization", 0.2)
        ...
        if self.stats.total_products > 0:
            ...
            avg_material_cost = sum(
                self.cost_parameters["material_cost_per_product"].values()
            ) / len(self.cost_parameters["material_cost_per_product"])
        ...
        quality_cost_score = (
            quality_cost_components["first_pass_rate"]
            * self.quality_cost_weights.get("first_pass_rate", 0.4)
            + quality_cost_components["cost_efficiency"]
            * self.quality_cost_weights.get("cost_efficiency", 0.6)
        ...
        agv_components = {
            "charge_strategy": min(
                100, kpis.charge_strategy_efficiency
            ),  # 已经是百分比
            "energy_efficiency": min(
                100, kpis.agv_energy_efficiency * 100
            ),  # 0.1 tasks/s = 100分 (每秒完成0.1个任务为满分)
            "utilization": min(100, kpis.agv_utilization),  # 已经是百分比
        }

        # AGV效率权重
        agv_score = (
            agv_components["charge_strategy"]
            * self.agv_weights.get("charge_strategy", 0.3)
            + agv_components["energy_efficiency"]
            * self.agv_weights.get("energy_efficiency", 0.4)
