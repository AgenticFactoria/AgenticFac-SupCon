...
class FaultSystem:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pending_agv_faults: Dict[
            str, FaultType
        ] = {}  # 新增：用于挂起对繁忙AGV的故障

        self.fault_definitions = {
            FaultType.STATION_FAULT: FaultDefinition(
                symptom="Station Vibration", min_duration=20.0, max_duration=60.0
            ),
            FaultType.AGV_FAULT: FaultDefinition(
                symptom="AGV Stuck", min_duration=20.0, max_duration=60.0
            ),
            FaultType.CONVEYOR_FAULT: FaultDefinition(
                symptom="Conveyor Stuck", min_duration=20.0, max_duration=60.0
            ),
        }

        # 故障注入参数
        self.fault_injection_interval = kwargs.get(
            "fault_injection_interval", (120, 300)
        )
    ...
    def inject_random_fault(
        ...
        if fault_type == FaultType.AGV_FAULT and device.status != DeviceStatus.IDLE:
            if target_device not in self.pending_agv_faults:
                ...
                print(
                    f"[{self.env.now:.2f}] ⚠️  AGV {target_device} is currently {device.status.value}, fault injection is pending."
                )
            else:
                print(
                    f"[{self.env.now:.2f}] ⚠️  AGV {target_device} already has a pending fault, skipping new injection."
                )
            return
    ...
    def _select_target_device(self, fault_type: FaultType) -> str:
        ...
        if fault_type == FaultType.AGV_FAULT:
            # AGV故障
            agv_devices = [
                dev_id for dev_id in self.factory_devices.keys() if "AGV" in dev_id
            ]
            ...
            conveyor_devices = [
                dev_id
                for dev_id in self.factory_devices.keys()
                if "Conveyor" in dev_id and "CQ" not in dev_id
            ]
            return (
                random.choice(conveyor_devices) if conveyor_devices else "Conveyor_AB"
            )
        else:
            # 工站故障
            station_devices = [
    ...
    def _inject_fault_now(
        self, device_id: str, fault_type: FaultType, duration: Optional[float] = None
    ):
        """立即注入故障的核心逻辑"""
        ...
        if hasattr(device, "interrupt_all_processing"):
            ...
            print(
                f"[{self.env.now:.2f}] 🚫 {device_id}: Interrupted {interrupted_count} processing operations"
            )
        # For other devices, interrupt the main action
        elif (
            hasattr(device, "action")
            and device.action
            and device.action.is_alive
            and device.action != self.env.active_process
        ):
            device.action.interrupt("Fault injected")

        device.set_status(DeviceStatus.FAULT)
        device.publish_status(
            f"[{self.env.now:.2f}] {device_id}: Fault injected: {fault.symptom}"
        )
        ...
        if self.kpi_calculator:
            ...
            self.kpi_calculator.add_maintenance_cost(
                device_id, fault.symptom, was_correct_diagnosis=True
            )
            ...
            if self.kpi_calculator and recovery_time > 0:
                ...
                if fault.fault_type == FaultType.AGV_FAULT:
                    self.kpi_calculator.update_agv_fault_time(
                        device_id, self.line_id, recovery_time
                    )
    ...
    def _send_fault_alert(self, device_id: str, fault: "SimpleFault"):
        ...
        if self.mqtt_client and self.topic_manager and self.line_id:
            ...
            self.mqtt_client.publish(topic, alert_data.json())
    ...
    def get_available_devices(self) -> List[str]:
        ...
        for device_id, device in self.factory_devices.items():
            if (
                device_id not in self.active_faults
                and device.status != DeviceStatus.FAULT
            ):
